...
for esm_ratio in esm_array:
    ...
    class TransConv1d(nn.Module):
        def __init__(self, input_size, output_size):
            ...
            self.transformer_encoder = nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=0.3, batch_first=True)
    ...
    # 保存结果的路径
    save_dir = f'/data/liuyuhuan/zhaijx/musite/model/{ptm_type}'
    ...
    for lambda_ in lambda_array:
        ...
        # all_test_logits = torch.cat(all_test_logits).cpu().numpy() 
        # all_test_labels = torch.cat(all_test_labels).cpu().numpy()

        # 保存测试集的结果
        # np.save(os.path.join(save_dir, f'test_logits_lambda_{lambda_}.npy'), all_test_logits)
        # np.save(os.path.join(save_dir, f'test_labels_lambda_{lambda_}.npy'), all_test_labels)
        ...
        # file.write(f"{ptm_type}:ACC: {test_acc:.4f}, AUC: {test_auc_score:.4f}, BACC: {test_bacc:.4f}, SN: {test_sn:.4f}, SP: {test_sp:.4f}, MCC: {test_mcc:.4f}, PR AUC: {test_pr_auc:.4f}\n")
        # file.write("\n")